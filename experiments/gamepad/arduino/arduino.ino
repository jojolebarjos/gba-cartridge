...
void loop() {
  ...
  // Read from SPI
  // Note: slave ignores the actual data, so let us just use zero
  uint16_t out = transfer(0x0000);

  // If state did change, notify
  if (out != keyinput) {
    keyinput = out;
    // TODO do an actual USB/BLE gamepad interface
